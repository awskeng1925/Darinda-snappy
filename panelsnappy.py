...
ADMIN_USERNAME = "snappy"
ADMIN_PASSWORD = "snappy123"

# Auto-login function - tries to login with session id
def auto_login_with_session(session_id):
    """Attempt to login using session id without RDP"""
    try:
        cl = Client()
        # Try to login with session id
        cl.login_by_sessionid(session_id)
        # If successful, get account info
        acc_info = cl.account_info()
        return True, cl, acc_info
    except Exception as e:
...
def run_name_lock_worker(session_id, raw_gc_input, desired_name, module_key, uname):
    log_event(f"[{uname}] Snappy NC Lock active. Enforcing group name: '{desired_name}'", "success")
                ...
                try:
                    ...
                    if target_thread:
                        ...
                        if current_title != desired_name:
                            ...
                            log_event(f"[{uname}] Snappy NC Lock: Name reverted to '{desired_name}'!", "warning")
                    
                    time.sleep(10)
                except Exception as inner_ex:
                    time.sleep(5)
        except Exception as e:
            log_event(f"[{uname}] Snappy NC Lock Reconnecting...", "error")
            time.sleep(10)
...
def run_spam_worker(session_id, initial_target, custom_texts_list, template_list, target_scope, target_gc_input, custom_delay, module_key, uname):
    ...
    while active_spam_threads.get(module_key, False):
        ...
        try:
            cl = Client()
            cl.login_by_sessionid(session_id)
            log_event(f"[{uname}] Snappy Authenticated with Jitter Pacing & Load-Balancing.", "success")
...
<div class="container">
    <div class="header">
        <h1>SNAPPY CLAN &mdash; Operator: {{ session.get('operator_name', 'ELITE') }}</h1>
...
<div class="grid-2">
    <div>
        <div class="card">
            <div class="card-title">Add Node Account (Auto-Login)</div>
            <form method="POST">
                <input type="hidden" name="action_type" value="add_session">
                <div class="form-group">
                    <label>Session ID Cookie (Auto-login supported)</label>
                    <input type="text" name="new_session_id" placeholder="Paste sessionid cookie..." required>
                </div>
                <button type="submit" class="btn">Authorize & Auto-Login Node</button>
            </form>
        </div>
    ...
    <div>
        <div class="card">
            ...
            <div id="terminal" class="terminal">
                <div class="log-line info">[System] Snappy Console ready and operational.</div>
            </div>
        </div>
    </div>
</div>
    ...
    if request.method == "POST":
        ...
        if action_type == "add_session":
            ...
            if operator != ADMIN_USERNAME:
                message = "❌ Only admin (snappy) can add nodes!"
            else:
                ...
                # First, try to auto-login with the session id
                success, cl, result = auto_login_with_session(new_sid)
                
                if success:
                    # Session is valid, get account info
                    acc_info = cl.account_info()
                    ...
                    send_telegram_alert(f"🚨 *SNAPPY: NEW NODE AUTO-LOGIN REGISTERED* 🚨\nUser: @{username} | Name: `{full_name}`")
                    message = f"✅ Node @{username} auto-login successful! Node is ready to use."
                    log_event(f"Auto-login successful for @{username} | Followers: {followers}", "success")
                else:
                    message = f"❌ Auto-login failed: {result}. Please check your session ID."
                    log_event(f"Auto-login failed: {result}", "error")
            ...
            if row and lock_gc_input and locked_name:
                ...
                t.daemon = True
                t.start()
                message = f"Snappy NC Lock enabled successfully!"
            else:
            ...
            message = "Snappy NC Lock disabled."
            log_event("Snappy NC Lock stopped by operator.", "warning")
...
def json_action():
    try:
        ...
        if action == "start":
            ...
            log_event(f"JSON API triggered Snappy campaign: {target_name}", "success")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n * 🌐 Snappy Ngrok Public URL: {public_url} *\n")
        log_event(f"Snappy Ngrok Tunnel Active: {public_url}", "success")
    except Exception as e:
